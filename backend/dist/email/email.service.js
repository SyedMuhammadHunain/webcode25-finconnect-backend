...
let EmailService = EmailService_1 = class EmailService {
    ...
    async sendEmail(email, passwordResetToken) {
    ...
    async emailSend(otp, email, resetLink) {
        let htmlContent = `
          <!DOCTYPE html>
              <html lang="en">
              <head>
    ...
    async isValidOtp(loginDto) {
        ...
        if (!found) {
            throw new common_2.UnauthorizedException('Cannot find OTP code for this email');
        }
        const { expiresAt } = found;
        if (expiresAt < new Date()) {
            await this.authCollections.deleteOne({ email }).exec();
            throw new common_2.UnauthorizedException('OTP code expired. Please request a new one.');
        }
        if (code !== found.code) {
            throw new common_2.UnauthorizedException('Invalid OTP code');
        }
        await this.authCollections.deleteOne({ email }).exec();
        return true;
    }
